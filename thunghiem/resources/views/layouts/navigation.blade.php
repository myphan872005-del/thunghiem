<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    {{-- LIÊN KẾT QUẢN LÝ TIN ĐĂNG CỦA TÔI --}}
                    @auth
                ...
                <div class="hidden sm:flex sm:items-center sm:ms-10 relative">
                    <form method="GET" action="{{ route('properties.indexSearch') }}" id="search-form" class="flex items-center space-x-4">
                        @push('scripts')
                        <script>
                            const BASE_URL = "{{ url('/') }}";
                        ...
                        <div class="nav-bar flex items-center space-x-4 flex-shrink-0">
                            <div class="mt-1">
                                ...
                                <select id="city-select" name="CityID" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm p-2 flex-shrink-0">
                                    <option value="">Chọn Tỉnh/Thành phố</option>
                                </select>
                            </div>
                            
                            <button type="button" id="toggle-adv-search" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:text-gray-500 focus:outline-none focus:border-blue-300 focus:ring focus:ring-blue-200 active:text-gray-800 active:bg-gray-50 disabled:opacity-25 transition w-36 justify-center"> 
                                Tìm Kiếm Nâng Cao
                            </button>
                        ...
                        <div id="adv-serach-content" class="advanced-search-container absolute z-20 top-16 mt-1 bg-white border border-gray-200 rounded-lg shadow-xl p-4 w-96 max-w-lg space-y-3 hidden" style="left: 0; top: 48px; border-top: 3px solid #4f46e5;">
                            ...
                            <div>
                                ...
                                <select id="ward-id" name="WardID" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm p-2">
                    ...
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                                ...
                                {{-- 🔥🔥🔥 SỬA PHẦN NÀY: HIỂN THỊ TÊN VÀ MÀU RANK TRÊN MENU 🔥🔥🔥 --}}
                                @php
                                    $rank = Auth::user()->rank_info;
                                @endphp
                                <div class="flex items-center {{ $rank['color'] ?? '' }} font-bold">
                                    {{ Auth::user()->name }}
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            @auth 
                <div class="px-4">
                    {{-- 🔥🔥🔥 SỬA PHẦN NÀY CHO MOBILE: HIỂN THỊ TÊN VÀ MÀU RANK 🔥🔥🔥 --}}
                    @php
                        $rank = Auth::user()->rank_info;
                    @endphp
                    <div class="font-medium text-base {{ $rank['color'] ?? 'text-gray-800' }} font-bold flex items-center">
                        {{ Auth::user()->name }}
                        <span class="ml-1">{{ $rank['icon'] ?? '' }}</span>
                    </div>
                    {{-- 🔥🔥🔥 KẾT THÚC PHẦN SỬA MOBILE 🔥🔥🔥 --}}
